...
export default function ClientCareLetterSalePage () {
  ...
  const [{ data: defaultData }] = useQuery({
    query: graphql(`
      query onboardingLettersClientCareLetterSale {
        me {
          id
          conveyancer {
            id
  ...
  useEffect(() => {
    if (defaultData) {
      setValue('client_care_letter_sale', defaultData.me?.conveyancer?.client_care_letter_sale ?? '')
    }
