...
const OnboardingLettersSigning = () => {
  const router = useRouter()

  const propertyId = router.query.id as string

  const [{ fetching, data, error }, createLettersSigningUrl] = useMutation(graphql(`
    mutation createLettersSigningUrl($property_id: ID!) {
      createLettersSigningUrl(property_id: $property_id)
    }
  `))

  useEffect(() => {
    createLettersSigningUrl({
      property_id: propertyId,
    })
  }, [createLettersSigningUrl, propertyId])
        ...
        <ClientPortalLayout>
          <ClientPortalLayout.MainContent>
            <div className="ml-[1.875rem] mr-[3.125rem] mt-[3.125rem]">
              ...
              <H1>Client Care Letter and Terms & Conditions</H1>
            </div>
                ...
                fetching
                  ...
                  : (
                    error
                      ...
                      : (
                        data?.createLettersSigningUrl
                          ? (
                            <iframe
                              title="onboarding-letters"
                              src={data?.createLettersSigningUrl}
...
export default OnboardingLettersSigning
